...
BIDIRECTIONAL_METRICS = ROOT / "working_files" / "train_shared_bidirectional" / "train_shared_bidirectional_files" / "shared_bidirectional_metrics.json"
BIDIRECTIONAL_METRICS_FALLBACK = ROOT / "working_files" / "train_shared_bidirectional" / "shared_bidirectional_metrics.json"
...
def pick(items: list[str], indexes: list[int]) -> list[str]:
    return [items[i] for i in indexes if i < len(items)]


def table(rows: list[list[object]], name: str) -> str:
    col_count = max((len(row) for row in rows), default=1)
    col_style = f"TableCol{min(col_count, 8)}"
...
bidirectional_metrics = load_json(BIDIRECTIONAL_METRICS) or load_json(BIDIRECTIONAL_METRICS_FALLBACK)
...
figures = [
    ...
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_loss_error.png", "Fig. 34. Shared bidirectional baseline loss trends."),
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_sequence_error.png", "Fig. 35. Shared bidirectional baseline sequence error trends."),
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_learning_rate.png", "Fig. 36. Shared bidirectional baseline learning-rate schedule."),
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_top_k_accuracy.png", "Fig. 37. Shared bidirectional baseline softmax-90 top-k accuracy."),
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_error_by_source_length.png", "Fig. 38. Shared bidirectional baseline error rate by source length."),
    ("working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_error_by_target_length.png", "Fig. 39. Shared bidirectional baseline error rate by target length."),
]

body: list[str] = []
body.append(p("Palestinian Arabizi and Arabic Transliteration", "Title"))
body.append(p("Deep Learning 535 Final Report", "Subtitle"))
body.append(p("This term paper studies Palestinian Arabizi-Arabic transliteration as a character-level sequence-to-sequence problem. The project compares a shared bidirectional Transformer, a dedicated Arabizi-to-Arabic model, direct Arabic-to-Arabizi baselines, and a two-stage Arabic-to-Arabizi pipeline that first predicts Arabic harakat/diacritics and then generates Arabizi. Results show that the dedicated Arabizi-to-Arabic model outperforms the shared bidirectional baseline, reaching 75.8 percent test sequence accuracy and 91.7 percent top-10 coverage. For Arabic-to-Arabizi, preserving five custom harakat hypotheses before Romanization gives the strongest practical result, reaching 64.1 percent test sequence accuracy and 98.3 percent top-10 coverage. The results support a pipeline design for the harder Arabic-to-Arabizi direction because ordinary Arabic omits pronunciation cues that Arabizi often represents.", "Abstract"))
...
intro = [
    ...
    "This paper introduces the task and related work, describes the dataset, presents the prototype models plus the top-one and Levanti comparison models, gives the evaluation metrics and results, and discusses error behavior and limitations. The goal is not to claim a finished research contribution or a ready operational app, but to show a complete deep-learning feasibility study: data preparation, model design, training, evaluation, and model comparison. The main engineering theme is candidate preservation. From an intelligence-use perspective, high softmax-90 top-k coverage is the key practical result because it improves the chance that a relevant spelling appears in a search or analyst review set. Rather than forcing one answer when the data itself supports several spellings, the model returns ranked transliteration candidates. This makes the prototype more honest and more useful for recall-oriented workflows.",
]
body.extend(p(x) for x in pick(intro, [0, 3, 4, 5]))
...
body.extend(p(x) for x in pick(related, [0, 2, 3]))
...
body.extend(p(x) for x in pick(dataset, [0, 1, 5, 6]))
...
modeldev = [
    "All transliteration components are character-level encoder-decoder Transformer models. The Arabizi-to-Arabic, original-harakat-to-Arabizi, no-harakat-to-Arabizi, Levanti-harakat-to-Arabizi, harakat-top-one-to-Arabizi, harakat-top-five-to-Arabizi, and shared bidirectional baseline models use a model dimension of 192, eight attention heads, three encoder layers, three decoder layers, a feed-forward dimension of 512, dropout of 0.15, and saved vocabularies for source and target characters. The harakat adder is slightly deeper and wider in the feed-forward block: it uses a model dimension of 192, six attention heads, four encoder layers, four decoder layers, and a feed-forward dimension of 768. This reflects the added complexity of predicting Arabic combining marks.",
    ...
    "The shared bidirectional baseline is included to test the obvious alternative: one Transformer checkpoint for both directions. It uses the same 192-dimensional encoder-decoder architecture as the transliteration models, but prefixes the source sequence with a direction token such as `<TO_ARABIC>` or `<TO_ARABIZI>`. Its Arabizi-to-Arabic examples map `arabizi` to `arabic_stripped`; its Arabic-to-Arabizi examples map `arabic_stripped` to `arabizi`. This makes it a fair direct bidirectional baseline, but it does not receive the top-five harakat candidate representation used by the selected two-stage pipeline.",
...
body.extend(p(x) for x in pick(modeldev, [0, 1, 2, 7, 8, 13, 14]))

bidirectional_final = bidirectional_metrics.get("test_metrics", {})
bidirectional_best_epoch = bidirectional_metrics.get("best_epoch", "pending run")
bidirectional_val = bidirectional_metrics.get("best_val_sequence_accuracy", "pending run")
bidirectional_test = bidirectional_final.get("sequence_accuracy", "pending run")
bidirectional_arabic_to_arabizi_test = bidirectional_final.get("arabic_to_arabizi_sequence_accuracy")
bidirectional_arabizi_to_arabic_test = bidirectional_final.get("arabizi_to_arabic_sequence_accuracy")
if isinstance(bidirectional_val, float):
    bidirectional_val = f"{bidirectional_val:.4f}"
if isinstance(bidirectional_test, float):
    bidirectional_test = f"{bidirectional_test:.4f}"
bidirectional_arabic_to_arabizi_text = f"{bidirectional_arabic_to_arabizi_test:.4f}" if isinstance(bidirectional_arabic_to_arabizi_test, float) else "pending run"
bidirectional_arabizi_to_arabic_text = f"{bidirectional_arabizi_to_arabic_test:.4f}" if isinstance(bidirectional_arabizi_to_arabic_test, float) else "pending run"
...
body.append(table([
    ...
    ["Shared bidirectional baseline", "direction token + arabizi/arabic_stripped", "arabic_stripped/arabizi", "3 enc / 3 dec", "8", bidirectional_best_epoch, bidirectional_val, bidirectional_test],
], "ModelSummary"))
body.append(caption("Table III. Model configurations and sequence-accuracy results from saved artifacts."))

body.append(h("Bidirectional Baseline", 2))
bidirectional_rationale = [
    "A shared bidirectional baseline was added so the project could directly test the one-model alternative instead of only arguing against it. The baseline is trained from both directions at once and uses a source-side direction token to tell the decoder which script to produce. This is similar to multilingual sequence-to-sequence training: the network shares parameters, while the control token specifies the desired output direction.",
    "This baseline is useful, but it is not equivalent to the selected pipeline. Arabizi-to-Arabic is mostly a normalization problem from an expressive Romanized spelling into Arabic script. Arabic-to-Arabizi is a reconstruction problem followed by generation: the model must infer missing harakat and then choose among several possible Arabizi spellings. Combining both directions into one model forces one training objective to cover two different information conditions.",
...
if bidirectional_metrics:
    bidirectional_rationale.append(
        "The shared baseline training artifact is stored at `working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_best.pt`, and its metrics are stored at `working_files/train_shared_bidirectional/train_shared_bidirectional_files/shared_bidirectional_metrics.json`. These values are loaded into Table III when the report is regenerated."
    )
    bidirectional_rationale.append(
        f"The completed shared model selected epoch {bidirectional_best_epoch} as its best validation checkpoint, with overall validation sequence accuracy {bidirectional_val}. On the test set it reached {bidirectional_test} overall sequence accuracy, split into {bidirectional_arabic_to_arabizi_text} for Arabic-to-Arabizi and {bidirectional_arabizi_to_arabic_text} for Arabizi-to-Arabic. This confirms that the shared model learned both tasks, but it lagged the specialized direction-specific models."
    )
else:
    bidirectional_rationale.append(
        "The reproducible notebook is `working_files/train_shared_bidirectional/train_shared_bidirectional.ipynb`. Running its full baseline cells writes `working_files/train_shared_bidirectional/train_shared_bidirectional_files/train_shared_bidirectional_best.pt` and `working_files/train_shared_bidirectional/train_shared_bidirectional_files/shared_bidirectional_metrics.json`; the report generator then fills the shared-baseline metrics in Table III automatically."
    )
body.extend(p(x) for x in pick(bidirectional_rationale, [0, 1, len(bidirectional_rationale) - 1]))

body.append(table([
    ["Shared baseline direction", "Test exact seq. acc.", "Top-1", "Top-3", "Top-5", "Top-10", "Interpretation"],
    ["Arabic to Arabizi", bidirectional_arabic_to_arabizi_text, "0.4668", "0.7059", "0.7685", "0.7970", "Below no-harakat and far below top-five harakat pipeline"],
    ["Arabizi to Arabic", bidirectional_arabizi_to_arabic_text, "0.7548", "0.8689", "0.8837", "0.8922", "Useful, but below the specialized Arabizi-to-Arabic model"],
], "SharedBidirectionalResults"))
body.append(caption("Table IV. Shared bidirectional baseline exact-match and softmax-90 top-k results."))
...
body.append(caption("Table V. Arabic-to-Arabizi experiment map showing the role of each trained model."))
...
body.extend(p(x) for x in pick(workflow, [0, 1, 2, 4]))
...
body.extend(p(x) for x in pick(evaltxt, [0, 1, 2]))

body.append(h("VIII. Results"))
body.append(p("The main results are easiest to read by separating the two transliteration directions and the intermediate harakat stage. The tables below emphasize top-k coverage because the intended intelligence-style use case is recall-oriented: a useful spelling only needs to survive into a compact candidate list for search expansion or analyst review. Exact sequence accuracy is still shown as a diagnostic, but the top-10 column is the most important practical number."))

body.append(table([
    ["Arabizi-to-Arabic approach", "How it was built", "Exact seq. acc.", "Top-1", "Top-3", "Top-5", "Top-10", "Takeaway"],
    ["Shared bidirectional model", "One Transformer with direction tokens; Arabizi half maps to `arabic_stripped`", "0.7001", "0.7548", "0.8689", "0.8837", "0.8922", "Works, but sharing capacity hurts"],
    ["Single-direction model", "Dedicated Transformer from `arabizi` to `arabic_stripped`", "0.7580", "0.7750", "0.8880", "0.9040", "0.9170", "Best Arabizi-to-Arabic result"],
], "ArabiziToArabicResultSummary"))
body.append(caption("Table VI. Arabizi-to-Arabic result summary emphasizing top-k coverage."))

body.append(table([
    ["Harakat-adding approach", "How it was built", "Top-1 harakat exact", "Top-k/coverage", "How it was used"],
    ["Custom harakat adder", "Project-trained character Transformer from `arabic_stripped` to `arabic_harakat`", "0.6453 overall prediction exact", "0.9499 target in softmax-90 candidates", "Generated custom top-1 and top-5 inputs for Arabizi models"],
    ["Levanti harakat adder", "External `guymorlan/levanti_arabic2diacritics` model applied to stripped Arabic", "0.2837 exact against project harakat", "not applicable; one external prediction", "Generated one Levanti-predicted input for the Arabizi model"],
], "HarakatAdderResultSummary"))
body.append(caption("Table VII. Harakat-adding result summary. The custom model produced ranked candidates; the Levanti comparison produced one prediction."))

body.append(table([
    ["Arabic-to-Arabizi approach", "Source given to Arabizi model", "Exact seq. acc.", "Top-1", "Top-3", "Top-5", "Top-10", "Takeaway"],
    ["Original Arabic with harakat", "`arabic_harakat`", "0.8312", "--", "--", "--", "--", "Upper-bound style result when gold pronunciation is visible"],
    ["Original Arabic without harakat", "`arabic_stripped`", "0.4738", "0.4650", "0.7510", "0.8260", "0.8740", "Fair baseline for ordinary Arabic input"],
    ["Levanti-added harakat", "`levanti_harakat_prediction`", "0.5016", "0.5060", "0.7810", "0.8400", "0.8860", "One external pronunciation hint helps modestly"],
    ["Custom harakat top-1", "`harakat_top_prediction`", "0.5152", "0.5050", "0.7610", "0.8210", "0.8750", "One custom pronunciation helps exact match but not top-10"],
    ["Custom harakat top-5", "`harakat_prediction_1..5`", "0.6409", "0.6540", "0.9220", "0.9670", "0.9830", "Best practical Arabic-to-Arabizi approach"],
], "ArabicToArabiziResultSummary"))
body.append(caption("Table VIII. Arabic-to-Arabizi result summary emphasizing top-k coverage for intelligence-style recall."))

body.append(h("Arabizi-to-Arabic Approaches", 2))
arabizi_to_arabic_results = [
    "The Arabizi-to-Arabic direction was tested in two ways: a shared bidirectional model and a single-direction model. Both use character-level Transformer encoder-decoder architecture, but they make different modeling assumptions. The shared model asks whether one checkpoint can learn both script directions at once. The single-direction model gives all capacity and all training signal to the Arabizi-to-Arabic normalization task.",
    "The shared bidirectional model was constructed by doubling the training examples and adding a direction token to the source sequence. An Arabizi input begins with `<TO_ARABIC>` and targets `arabic_stripped`; an Arabic input begins with `<TO_ARABIZI>` and targets `arabizi`. This is similar to multilingual sequence-to-sequence training, where a control token tells the decoder what language or script to produce. It was included because a reader might reasonably ask why the project did not simply train one model for both directions. The result was useful but weaker than the specialized alternatives: overall test sequence accuracy was 0.5755, with 0.7001 on the Arabizi-to-Arabic half. Its Arabizi-to-Arabic softmax-90 top-k coverage was 0.7548 at top-1, 0.8689 at top-3, 0.8837 at top-5, and 0.8922 at top-10.",
    "The single-direction Arabizi-to-Arabic model was constructed as a direct transliterator from the Maknuune `arabizi` column to `arabic_stripped`. This model is simpler than the bidirectional baseline because it does not have to share parameters with Arabic-to-Arabizi. That design is justified because Arabizi-to-Arabic is mostly a normalization problem: Latin vowels, doubled letters, and numerals such as 2, 3, and 7 often reveal the user's intended pronunciation. The model completed 100 epochs and selected epoch 89 as the best checkpoint by validation sequence accuracy. It reached 0.7580 test sequence accuracy, with top-k coverage of 0.775 at top-1, 0.888 at top-3, 0.904 at top-5, and 0.917 at top-10. This outperformed the Arabizi-to-Arabic side of the shared bidirectional model, supporting the choice to keep a dedicated reverse-normalization model.",
]
body.extend(p(x) for x in arabizi_to_arabic_results)

body.append(h("Arabic-to-Arabizi Without Added Harakat", 2))
no_added_harakat_results = [
    "The Arabic-to-Arabizi direction was first tested without any predicted harakat stage. This group contains two important comparisons: a model trained with pre-existing gold harakat and a model trained without harakat. Together they measure how much visible pronunciation information matters.",
    "The model trained with pre-existing harakat uses `arabic_harakat` as its source and `arabizi` as its target. This is the pronunciation-visible upper-bound condition: it asks how well the Transformer can Romanize Arabic when the short vowels, shadda, sukun, and other marks are already present. The architecture is the same 192-dimensional character-level Transformer used by the other transliteration models. It reached 0.8312 test sequence accuracy, the strongest one-best Arabic-to-Arabizi result in the project. However, this is not the normal user workflow because ordinary users usually type Arabic without full harakat.",
    "The same gold-harakat-trained model was also stress-tested on stripped Arabic input. That test performed badly: top-1 was 0.084, top-3 was 0.168, top-5 was 0.220, and top-10 was 0.309. This result is important because it shows that a model trained to expect harakat cannot simply be handed unvocalized Arabic at inference time. The input representation changed too much.",
    "The model trained without harakat uses `arabic_stripped` as its source and `arabizi` as its target. This is the clean baseline for ordinary written Arabic. It selected epoch 88 as the best checkpoint and reached 0.4738 test sequence accuracy. Its candidate coverage was much better than its single-best accuracy: top-1 was 0.465, top-3 was 0.751, top-5 was 0.826, and top-10 was 0.874. The model can infer many short and common forms directly, but it lacks explicit pronunciation evidence, so it falls well short of the gold-harakat model and the final top-five pipeline.",
]
body.extend(p(x) for x in no_added_harakat_results)

body.append(h("Arabic-to-Arabizi With Custom Harakat Added", 2))
custom_harakat_results = [
    "The next group adds harakat before Arabizi generation. The first stage is the project's own harakat adder, trained from `arabic_stripped` to `arabic_harakat` on the same Palestinian Maknuune-derived distribution as the downstream transliteration models. Architecturally, it is a character-level Transformer encoder-decoder like the transliteration models, but slightly deeper and wider in the feed-forward block: model dimension 192, six attention heads, four encoder layers, four decoder layers, feed-forward size 768, and dropout 0.15. This model restores Arabic letters plus combining marks one character at a time.",
    "The harakat adder is not used as a final user-facing model. It is an intermediate pronunciation model. Its best validation checkpoint was epoch 83, with validation sequence accuracy 0.4983 and validation loss 0.2680. It performed best on short-to-medium strings: 10-19 character inputs reached about 0.5710 validation sequence accuracy, while longer inputs dropped sharply. That performance is imperfect, but useful for this pipeline because the downstream model can consume more than one candidate rather than depending on a single forced vocalization.",
    "The custom top-one-to-Arabizi model uses only `harakat_top_prediction`, the single most likely output from the custom harakat adder, as the source for Arabizi generation. It was trained as an ablation: if the top-one model performed nearly as well as top-five, the simpler pipeline might be enough. It selected epoch 78 as the best checkpoint and reached about 0.5152 test sequence accuracy. Its softmax-90 top-k coverage was 0.505 at top-1, 0.761 at top-3, 0.821 at top-5, and 0.875 at top-10. This improved one-best exact match over the no-harakat baseline, but top-10 coverage remained almost the same, showing that one guessed vocalization does not preserve enough uncertainty.",
    "The custom top-five-to-Arabizi model uses up to five harakat hypotheses from the custom adder as the source representation. This is the selected Arabic-to-Arabizi pipeline because it keeps several plausible pronunciations alive before Romanization. It selected epoch 98 as the best checkpoint and reached 0.6409 test sequence accuracy. Its softmax-90 coverage was 0.654 at top-1, 0.922 at top-3, 0.967 at top-5, and 0.983 at top-10. This is the strongest practical result for ordinary Arabic input: it starts from unvocalized Arabic, adds project-specific Palestinian harakat candidates, and returns a high-recall Arabizi candidate list.",
]
body.extend(p(x) for x in custom_harakat_results)

body.append(h("Arabic-to-Arabizi With Levanti Harakat Added", 2))
levanti_results = [
    "The Levanti-harakat approach uses an outside diacritization stage instead of the project's own harakat adder. The external model was `guymorlan/levanti_arabic2diacritics`, a Hugging Face Levantine/Palestinian-style Arabic diacritizer. Its internal architecture was not trained or modified in this project; it was used as a ready-made dialect-aware harakat predictor. The downstream Arabizi generator was still the same project architecture: a 192-dimensional character-level Transformer encoder-decoder trained from `levanti_harakat_prediction` to `arabizi`.",
    "This approach tests whether a general Levantine pronunciation hint is enough for the final Arabizi task. It is effectively a Levanti top-one path because only one Levanti-predicted vocalized form is passed forward. The downstream model selected epoch 98 as the best checkpoint, with validation sequence accuracy 0.4932. On test data it reached about 0.5016 sequence accuracy. Its top-k coverage was 0.506 at top-1, 0.781 at top-3, 0.840 at top-5, and 0.886 at top-10.",
    "The Levanti path was helpful as an external comparison, but it did not match the custom top-five pipeline. Its one-best exact match was slightly better than the no-harakat baseline and similar to the custom top-one setup, but its top-10 coverage was far below the custom top-five model's 0.983. The most likely explanation is not that Levanti is poor; it is that the final task benefits from a harakat stage trained on the same Maknuune-derived Palestinian data and, more importantly, from preserving multiple pronunciation hypotheses rather than collapsing the upstream uncertainty to one string.",
]
body.extend(p(x) for x in levanti_results)

body.append(h("Overall Result Pattern", 2))
overall_results = [
    "The results form a readable ladder. For Arabizi-to-Arabic, a dedicated single-direction model works better than the shared bidirectional model because the task is mostly normalization from an expressive Romanized spelling into Arabic script. For Arabic-to-Arabizi, the model with gold harakat proves that the Transformer can Romanize well when pronunciation is visible, while the no-harakat baseline shows the penalty for ordinary unvocalized Arabic. Adding one predicted harakat string helps only modestly, whether that string comes from the custom adder or the Levanti model. Preserving five custom harakat hypotheses is the strongest approach because it gives the Arabizi generator multiple plausible pronunciations before asking it to choose Latin-script spellings.",
    "From the intelligence-workflow perspective, the top-k results are the headline finding. The single-best exact-match numbers show how often the model chooses the reference spelling by itself. The softmax-90 top-k numbers show whether the useful spelling survives into a candidate list for search expansion, analyst review, or downstream NLP. For Arabic-to-Arabizi, where missing harakat and non-standard spelling make one-best prediction inherently uncertain, top-10 coverage is a better practical metric than top-1 exact match alone.",
]
body.extend(p(x) for x in overall_results)

body.append(table([
    ["Model", "Top-1", "Top-3", "Top-5", "Top-10", "Sample size"],
    ["Shared bidirectional, Arabic to Arabizi", "0.467", "0.706", "0.769", "0.797", "527"],
    ["Shared bidirectional, Arabizi to Arabic", "0.755", "0.869", "0.884", "0.892", "473"],
...
body.append(caption("Table IX. Softmax-90 top-k coverage for candidate-generating transliteration models."))
...
body.extend(p(x) for x in pick(analysis, [0, 2, 5, 7]))
...
body.append(caption("Table X. Selected sequence-accuracy buckets from length-based error analysis."))

body.append(h("X. Threats to Validity and Feasibility Boundaries"))
validity = [
    "Because this is a prototype, the evaluation should be read as evidence of feasibility rather than proof of readiness for operational deployment. The strongest limitation is dataset realism. Maknuune is a clean and valuable Palestinian Arabic lexical resource, and it was the best available dataset found for this project because it directly connects Palestinian Arabic, Arabizi, and diacritized Arabic forms. However, it is not the same as noisy real-world intelligence text. Social-media and messaging-style Arabizi can contain full sentences, emojis, hashtags, usernames, code-switching with English, spelling mistakes, repeated letters, sarcasm, abbreviations, and irregular punctuation. The present system is strongest on words and short phrases that resemble the lexicon-style training examples. A production system would need an external test set drawn from realistic Arabizi data and would need preprocessing for noisy text.",
    "The intelligence framing also needs careful boundaries. This system is not an intelligence system and should not be treated as one. It does not infer intent, threat, identity, reliability, location, or meaning. It only attempts to normalize script forms so that Arabizi material can be searched or routed into Arabic-language tools. In a responsible intelligence workflow, transliteration would be one early text-processing step used with lawful data handling, human review, context, and policy controls. The model can increase recall for keyword monitoring, but it cannot decide whether a keyword match matters.",
...
body.extend(p(x) for x in pick(validity, [0, 2, 3, 4, 6]))
...
practical = [
    ...
    "The shared bidirectional baseline is useful mainly as negative evidence. It reached 57.5 percent overall test exact-match accuracy, with 45.1 percent for Arabic-to-Arabizi and 70.0 percent for Arabizi-to-Arabic. Its top-10 coverage was 79.7 percent for Arabic-to-Arabizi and 89.2 percent for Arabizi-to-Arabic. These are respectable results for one shared checkpoint, but they are weaker than the specialized Arabizi-to-Arabic model and much weaker than the selected top-five harakat Arabic-to-Arabizi pipeline.",
...
body.extend(p(x) for x in pick(practical, [0, 2]))
...
body.extend(p(x) for x in pick(limits, [0, 1, 4]))

body.append(h("XIV. Conclusion"))
conclusion = [
    "This project produced a working proof-of-concept model pipeline for Palestinian Arabizi-Arabic transliteration built from cleaned Maknuune data, one custom Arabizi-to-Arabic model, five Arabic-to-Arabizi modeling paths, a shared bidirectional baseline, a custom harakat adder, saved metrics, and plotted diagnostics. It is not a complete app ready for full operational use; it is a feasibility demonstration that shows a promising architecture and a clear path for later refinement with more time, data, validation, and engineering resources. The central argument is that the challenge is not simply moving between two scripts. It is the Arabic-to-Arabizi direction specifically: ordinary Arabic omits many harakat and therefore hides pronunciation cues, while Arabizi itself allows many writer-dependent spellings. Arabizi-to-Arabic reaches 91.7 percent top-10 coverage on the sampled softmax-90 evaluation, but Arabic-to-Arabizi requires explicit uncertainty handling.",
    ...
    "The report also shows why the system is architected as a pipeline rather than one shared model. Arabic without harakat does not fully specify pronunciation, while Arabizi often represents vowels. The shared bidirectional baseline did learn both directions, but it reached only 79.7 percent top-10 coverage for Arabic-to-Arabizi and 89.2 percent for Arabizi-to-Arabic. By contrast, the no-harakat model reaches 87.4 percent top-10 coverage, the Levanti comparison reaches 88.6 percent, the custom top-one comparison reaches 87.5 percent, and the harakat-top-five-to-Arabizi stage reaches 98.3 percent. Those softmax-90 top-k results are the most important practical intelligence result in the paper because they measure whether the right candidate survives into a usable review/search set. The metrics and graphics give clear targets for future deep-learning improvement.",
]
body.extend(p(x) for x in conclusion)

body.append(h("XV. Key Terms"))
key_terms = [
    "Arabizi. Arabizi is informal Arabic written with Latin letters and numerals instead of Arabic script. For example, writers may use 3 for ع or 7 for ح because those Arabic sounds do not have simple Latin-letter equivalents.",
    "Arabic script. Arabic script is the writing system normally used for Arabic, written from right to left. In this project, Arabic-script forms are compared with Arabizi forms written in Latin characters and numerals.",
    "Harakat / Arabic diacritics. In the Arabic context, harakat are diacritics: small marks above or below letters that indicate short vowels, vowel absence, consonant doubling, and related pronunciation information. This report uses harakat and Arabic diacritics to mean the same thing.",
    "Undiacritized or unvocalized Arabic. Undiacritized Arabic is Arabic script after the harakat/diacritics have been removed. This resembles ordinary Arabic writing, where many short-vowel cues are usually not shown.",
    "Diacritized or vocalized Arabic. Diacritized Arabic is Arabic script with harakat included. In this project, it corresponds to the dataset field that preserves pronunciation marks before they are stripped away.",
    "Diacritization. Diacritization is the task of adding harakat/diacritics back to Arabic text. In this project, the harakat adder performs diacritization before the Arabic-to-Arabizi model generates Romanized candidates.",
    "Transliteration. Transliteration converts text from one writing system to another while preserving approximate pronunciation. Here, the two transliteration directions are Arabic script to Arabizi and Arabizi back to Arabic script.",
    "Romanization. Romanization means writing a language normally written in another script using Latin-script characters. Arabizi is an informal, non-standard Romanized style of Arabic.",
    "Dialectal Arabic. Dialectal Arabic refers to everyday spoken varieties of Arabic, which differ by region in pronunciation, vocabulary, and grammar. This matters because Arabizi spellings often reflect spoken dialect rather than formal written Arabic.",
    "Palestinian Arabic. Palestinian Arabic is the dialect focus of this project. The models are trained on Palestinian-focused data, so their behavior should not be assumed to generalize to all Arabic dialects.",
    "Hamza and alif variation. Hamza and alif are Arabic orthographic elements whose written forms can vary across words and conventions. Small differences in these forms can change exact-match scoring even when outputs remain understandable to a reader.",
]
body.extend(p(term) for term in key_terms)
...
<office:document-content
 ...
 <office:automatic-styles>
  ...
  <style:style style:name="TableCol1" style:family="table-column"><style:table-column-properties style:column-width="6.50in" style:rel-column-width="65535*"/></style:style>
  <style:style style:name="TableCol2" style:family="table-column"><style:table-column-properties style:column-width="3.25in" style:rel-column-width="32767*"/></style:style>
  <style:style style:name="TableCol3" style:family="table-column"><style:table-column-properties style:column-width="2.17in" style:rel-column-width="21845*"/></style:style>
  <style:style style:name="TableCol4" style:family="table-column"><style:table-column-properties style:column-width="1.62in" style:rel-column-width="16384*"/></style:style>
  <style:style style:name="TableCol5" style:family="table-column"><style:table-column-properties style:column-width="1.30in" style:rel-column-width="13107*"/></style:style>
  <style:style style:name="TableCol6" style:family="table-column"><style:table-column-properties style:column-width="1.08in" style:rel-column-width="10922*"/></style:style>
  <style:style style:name="TableCol7" style:family="table-column"><style:table-column-properties style:column-width="0.93in" style:rel-column-width="9362*"/></style:style>
  <style:style style:name="TableCol8" style:family="table-column"><style:table-column-properties style:column-width="0.81in" style:rel-column-width="8192*"/></style:style>
 </office:automatic-styles>
 <office:body>
  <office:text>
   {"".join(body)}
